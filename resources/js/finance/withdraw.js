...
function applyWithdraw(){
    ...
    getAjax("GET","","income/auth/getFrozenAmount",getYesterdayIncomeCallBack,true);
}

function getYesterdayIncomeCallBack(data){
    $("#frozenAmount").html(data);
    $("#frozenAmount1").val(data);
}
...
//        content+="<td>"+null+"</td>";
        ...
        if(item.statusName=="失败"){
            content+="<td style='color: red; cursor: pointer' onclick='showCheckDetail(this)'><u>"+item.statusName+"</u></td>";

        }else{
            content+="<td style='color: blue'>"+item.statusName+"</td>";
        }
        content+="<input type='hidden' value='"+item.checkContent+"'></input></tr>";
